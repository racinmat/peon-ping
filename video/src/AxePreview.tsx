...
const TIMELINE = [
  ...
  { frame: 120, type: "sound-line" as const, text: '🔊 "Axe is ready!"', sound: "AxeIsReady.mp3", label: "— session started" },
  { frame: 170, type: "line" as const, text: "> Migrate the database to v3 schema", style: "cmd" as const },
  { frame: 210, type: "line" as const, text: "  Claude is working...", style: "dim" as const },
  { frame: 240, type: "sound-line" as const, text: '🔊 "To battle!"', sound: "ToBattle.mp3", label: "— reading files" },
  { frame: 290, type: "line" as const, text: "  [you switch to Slack]", style: "dim" as const },
  { frame: 330, type: "sound-line" as const, text: '🔊 "I said good day, sir!"', sound: "ISaidGoodDaySir.mp3", label: "— permission needed" },
  ...
  { frame: 470, type: "sound-line" as const, text: '🔊 "Forward!"', sound: "Forward.mp3", label: "— analyzing code" },
  { frame: 530, type: "sound-line" as const, text: '🔊 "What happened? Axe happened!"', sound: "AxeHappened.mp3", label: "— task complete" },
  { frame: 580, type: "line" as const, text: "> ", style: "cursor" as const },
  { frame: 620, type: "line" as const, text: "> Push untested code to prod", style: "cmd" as const },
  { frame: 660, type: "line" as const, text: "  Error: Pipeline failed", style: "error" as const },
  { frame: 680, type: "sound-line" as const, text: '🔊 "You fought badly. Died worse."', sound: "FoughtBadly.mp3", label: "— error" },
  { frame: 740, type: "outro" as const },
];

// Dota 2 Axe palette — blood red + steel
const BG = "#1a1b26";
const BAR_BG = "#0c0d14";
const GREEN = "#4ade80";
const WC3_GOLD = "#ffab01";
const WC3_GOLD_DIM = "rgba(255, 171, 1, 0.3)";
const AXE_RED = "#c0392b";
const DIM = "#505a79";
const BRIGHT = "#e0e8ff";
...
const TerminalChrome: React.FC<{ children: React.ReactNode; tabTitle: string }> = ({ children, tabTitle }) => (
  <div style={{ width: 940, borderRadius: 12, overflow: "hidden", border: "1px solid rgba(192,57,43,0.15)", boxShadow: "0 0 60px rgba(0,0,0,0.6), 0 0 4px rgba(192,57,43,0.1)" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 18px", backgroundColor: BAR_BG, borderBottom: "1px solid rgba(192,57,43,0.1)" }}>
...
const TitleCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f", justifyContent: "center", alignItems: "center", opacity: exitOp }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${AXE_RED}, transparent)` }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${AXE_RED}, transparent)` }} />
      <div style={{ position: "absolute", top: "50%", left: "50%", width: 600, height: 600, transform: "translate(-50%, -50%)", borderRadius: "50%", background: "radial-gradient(circle, rgba(192,57,43,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", opacity: titleSpring, transform: `translateY(${interpolate(titleSpring, [0, 1], [20, 0])}px)` }}>
        <div style={{ fontFamily: "monospace", fontSize: 20, color: AXE_RED, letterSpacing: 6, textTransform: "uppercase", marginBottom: 16, opacity: subSpring }}>sound pack</div>
        <div style={{ fontFamily: "Georgia, 'Palatino Linotype', serif", fontSize: 86, fontWeight: 700, color: "#fff", textShadow: "3px 3px 0 rgba(0,0,0,0.8)", textAlign: "center", lineHeight: 1.2 }}>Axe</div>
        <div style={{ fontFamily: "Georgia, serif", fontSize: 36, color: "rgba(255,255,255,0.5)", marginTop: 12, opacity: subSpring }}>Dota 2</div>
      </div>
...
const OutroCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f", justifyContent: "center", alignItems: "center" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${AXE_RED}, transparent)` }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${AXE_RED}, transparent)` }} />
      <div style={{ position: "absolute", top: "40%", left: "50%", width: 500, height: 500, transform: "translate(-50%, -50%)", borderRadius: "50%", background: "radial-gradient(circle, rgba(192,57,43,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
...
export const AxePreview: React.FC = () => {
  const frame = useCurrentFrame();
                ...
                return (
                  <TermLine key={i} appearFrame={event.frame}>
                    {event.style === "cmd" ? (
                      ...
                      <span style={{ color: AXE_RED }}>{event.text}</span>
                    ) : event.style === "cursor" ? (
